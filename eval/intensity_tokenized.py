...
def get_intensities(model, tokenizer, panel, val_loader, ch2stain, calculate_ssims=False, device='cpu', return_meta=False, distributed_state=None, T=1, temp=0):
    ...
    for i, batch in enumerate(tqdm(val_loader)):
        ...
        with torch.no_grad():
            ...
            out, token_ids, labels = model.predict(token_ids, masked_ch_idx=masked_ch_idx, T=T, temp=temp)
            
            # Detokenize back to images
            preds = tokenizer.detokenize(token_ids).reshape(gt.shape)
